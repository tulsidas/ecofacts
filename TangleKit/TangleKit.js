...
//----------------------------------------------------------
//
//  Multipictures - tulsiagregado
//
Tangle.classes.MultiPictures = { 
   initialize: function (element, options, tangle, variable) { 
      // element: el <span>
      // options: los atributos del tipo "data-"
   ...
   update: function (element, value) {
      ...
      // paste here the old code from v_multiPictures.  For example: 
      var imgCount = Math.ceil(value).limit(1, options.maxImageCount); 
      var imgAspect = options.imageHeight / options.imageWidth; 
      var viewWidth = options.viewWidth; 
      var viewHeight = options.viewHeight; 
      // etc. 
      // If you want to set the options in the HTML, you'll need to change 
      // the option names so they aren't CamelCase.  Or you can just set 
      // the options here in the JavaScript. 
      */


      // TODO saco todo y repongo cada vez
      var el = jQuery(element);
      el.empty();
      
      // pongo _value_ imagenes
      _.times(value, function() {
         var s = $('<span>');
         s.text("hola! ");
         s.attr("class", "picante");
...
//  TKMultiToggle
//
//  Click to toggle value between [0, childCount]

Tangle.classes.TKMultiToggle = {

    initialize: function (element, options, tangle, variable) {
        element.addEvent("click", function (event) {
            var current = tangle.getValue(variable);
            var size = element.getChildren().length;
            tangle.setValue(variable, current + 1 == size ? 0 : current + 1);
        });
    }
};
...
Tangle.formats.metric = function (value, unit) {
   var division;
   var modifier;
   if (value >= 1e12) {
      division = 1e12;
   ...
   return Highcharts.numberFormat((value / division), 2) + " " + modifier + unit;
};

Tangle.formats.metric_word = function (value, unit) {
   var division;
   var modifier;
   if (value >= 1e12) {
      division = 1e12;
